...
namespace ConsoleIO
{
    ...
    public static class ConsoleIO
    {
        ...
        // If found any char that are double space but not handled, add it here
        private static Regex CJKDoubleSpace = new Regex(@"[\p{IsCJKUnifiedIdeographs}，；：｛｝（）！？]");
        ...
        private static void RemoveOneChar()
        {
            if (buffer.Length > 0)
            {
                try
                {
                    ...
                    {
                        Console.Write("\b \b");
                        if (buffer[buffer.Length - 1].IsDoubleSpace())
                            Console.Write("\b \b");
                    }
                }
        ...
        private static void AddChar(char c)
        {
            ...
            {
                GoBack();
                if (buffer2[buffer2.Length - 1 - i].IsDoubleSpace())
                    GoBack();
            }
        }

        private static bool IsDoubleSpace(this string c)
        {
            return CJKDoubleSpace.IsMatch(c);
        }
        private static bool IsDoubleSpace(this char c)
        {
            return IsDoubleSpace(c.ToString());
        }
